  ...
  if (req.isAuthenticated() === true) {
    Record.find({ 'username': req.user.username }, function (err, recordResult) {
      if (err) {
        ...
        // res.render('error')
        res.render('userProfile', { noActive: noActive, noTest: noTest, hasRecord: hasRecord, user: req.user, loginStatus: req.isAuthenticated() })
      }
    })
  } else {
    res.render('error')
        ...
        Record.findOne({'username': req.user.username, 'lesson.lessonId': req.params.lessonId}, {'lesson.$': 1}, function (err, recordResult) {
          if (err) {
            ...
            if (!recordResult) {
              ...
              var testRecord = (recordResult.lesson[0].testRecord).toString()// testRecord：某一課程所有考試成績
              console.log('perLessonRecord')
              console.log(perLessonRecord)
              console.log('testRecord')
              console.log(testRecord)
            }
            res.render('allRecord', { recordInform: recordList, lessonId: req.params._id, noActive: noActive, noTest: noTest, hasRecord: hasRecord, perLessonRecord: perLessonRecord, testRecord: testRecord, user: req.user, loginStatus: req.isAuthenticated() })
          }
        })
      }
    })
